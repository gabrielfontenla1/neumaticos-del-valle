 ...
 *
 * This page provides obfuscated access to the admin login.
 * Requires a secret access key via URL parameter to show the login form.
 * Without the correct key, redirects to 404 (fake not found).
 *
 * Access URL: /panel?acc=YOUR_SECRET_KEY
 */
'use client'
...
function PanelContent() {
  ...
  useEffect(() => {
    const key = searchParams.get('acc')
    if (key === ACCESS_KEY) {
      setAuthorized(true)
    } else {
      // Redirect to 404 after a small delay (simulate loading)
