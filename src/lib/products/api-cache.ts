 ...
 * These represent typical user queries
 */
export const COMMON_FILTER_COMBINATIONS = [
  // Popular brands
  { selectedBrand: 'FIRESTONE' },
  { selectedBrand: 'GOODYEAR' },
  { selectedBrand: 'MICHELIN' },

  // Popular categories
  { selectedCategory: 'AUTO' },
  { selectedCategory: 'CAMIONETA' },

  // Popular sizes
  { selectedWidth: '205', selectedProfile: '55', selectedDiameter: '16' },
  { selectedWidth: '195', selectedProfile: '65', selectedDiameter: '15' },
  { selectedWidth: '215', selectedProfile: '60', selectedDiameter: '16' },

  // Combinations
  { selectedBrand: 'FIRESTONE', selectedCategory: 'AUTO' },
  { selectedBrand: 'GOODYEAR', selectedCategory: 'CAMIONETA' },
] as const

/**
 * Warms the cache with common filter combinations
 * Should be called periodically or after cache clear
 */
export async function warmCache(
  fetchFunction: (filters: Partial<FilterState>) => Promise<APIResponse>
): Promise<void> {
  const warmingPromises = COMMON_FILTER_COMBINATIONS.map(async (filters) => {
    try {
      const key = apiCache.generateKey(filters)

      // Only warm if not already cached
      if (!apiCache.get(key)) {
        const data = await fetchFunction(filters)
        apiCache.set(key, data, filters)
      }
    } catch (error) {
...
/**
 * Utility to check if caching is enabled
 */
...
/**
 * Utility to get cache configuration
 */
